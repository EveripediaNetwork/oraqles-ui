export const marchMadnessAbi = [
  {
    anonymous: false,
    inputs: [
      { indexed: false, internalType: 'string', name: 'id', type: 'string' },
      {
        indexed: false,
        internalType: 'string',
        name: 'homeTeam',
        type: 'string',
      },
      {
        indexed: false,
        internalType: 'string',
        name: 'awayTeam',
        type: 'string',
      },
      {
        indexed: false,
        internalType: 'uint256',
        name: 'winner',
        type: 'uint256',
      },
      {
        indexed: false,
        internalType: 'uint256',
        name: 'homePoints',
        type: 'uint256',
      },
      {
        indexed: false,
        internalType: 'uint256',
        name: 'awayPoints',
        type: 'uint256',
      },
      {
        indexed: false,
        internalType: 'uint256',
        name: 'scheduled',
        type: 'uint256',
      },
    ],
    name: 'CallWinner',
    type: 'event',
  },
  {
    anonymous: false,
    inputs: [
      {
        indexed: true,
        internalType: 'address',
        name: 'previousOwner',
        type: 'address',
      },
      {
        indexed: true,
        internalType: 'address',
        name: 'newOwner',
        type: 'address',
      },
    ],
    name: 'OwnershipTransferred',
    type: 'event',
  },
  {
    anonymous: false,
    inputs: [
      {
        indexed: false,
        internalType: 'string',
        name: 'ipfsHash',
        type: 'string',
      },
    ],
    name: 'SetIPFS',
    type: 'event',
  },
  {
    inputs: [{ internalType: 'string', name: '', type: 'string' }],
    name: 'allGames',
    outputs: [
      { internalType: 'string', name: 'id', type: 'string' },
      { internalType: 'string', name: 'homeTeam', type: 'string' },
      { internalType: 'string', name: 'awayTeam', type: 'string' },
      { internalType: 'uint256', name: 'winner', type: 'uint256' },
      { internalType: 'uint256', name: 'homePoints', type: 'uint256' },
      { internalType: 'uint256', name: 'awayPoints', type: 'uint256' },
      { internalType: 'uint256', name: 'scheduled', type: 'uint256' },
      { internalType: 'uint256', name: 'round', type: 'uint256' },
    ],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [
      { internalType: 'uint256', name: 'round', type: 'uint256' },
      { internalType: 'string', name: 'id', type: 'string' },
      { internalType: 'string', name: 'homeTeam', type: 'string' },
      { internalType: 'string', name: 'awayTeam', type: 'string' },
      { internalType: 'uint256', name: 'winner', type: 'uint256' },
      { internalType: 'uint256', name: 'homePoints', type: 'uint256' },
      { internalType: 'uint256', name: 'awayPoints', type: 'uint256' },
      { internalType: 'uint256', name: 'scheduled', type: 'uint256' },
    ],
    name: 'callWinner',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{ internalType: 'string', name: 'id', type: 'string' }],
    name: 'getGame',
    outputs: [
      {
        components: [
          { internalType: 'string', name: 'id', type: 'string' },
          { internalType: 'string', name: 'homeTeam', type: 'string' },
          { internalType: 'string', name: 'awayTeam', type: 'string' },
          { internalType: 'uint256', name: 'winner', type: 'uint256' },
          { internalType: 'uint256', name: 'homePoints', type: 'uint256' },
          { internalType: 'uint256', name: 'awayPoints', type: 'uint256' },
          { internalType: 'uint256', name: 'scheduled', type: 'uint256' },
          { internalType: 'uint256', name: 'round', type: 'uint256' },
        ],
        internalType: 'struct MarchMadness2021.Game',
        name: '',
        type: 'tuple',
      },
    ],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [],
    name: 'ipfsFullData',
    outputs: [{ internalType: 'string', name: '', type: 'string' }],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [],
    name: 'owner',
    outputs: [{ internalType: 'address', name: '', type: 'address' }],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [],
    name: 'renounceOwnership',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{ internalType: 'string', name: 'ipfsHash', type: 'string' }],
    name: 'setIpfsData',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{ internalType: 'address', name: 'newOwner', type: 'address' }],
    name: 'transferOwnership',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
]
